...
def generate_slurm_script(config, subject, session, path_to_script, job_ids=None):
    """
    Generate the SLURM job script for QC QSIrecon processing.
    ...
    header = (
        f'#!/bin/bash\n'
        f'#SBATCH --job-name=qc_qsirecon_{subject}_{session}\n'
        f'#SBATCH --output={DERIVATIVES_DIR}/qc/qsirecon/stdout/qc_qsirecon_{subject}_{session}_%j.out\n'
        f'#SBATCH --error={DERIVATIVES_DIR}/qc/qsirecon/stdout/qc_qsirecon_{subject}_{session}_%j.err\n'
    ...
    prereq_check = (
        f'\n# Check that QSIRECON outputs exists\n'
        f'if [ ! -d "{DERIVATIVES_DIR}/qsirecon/outputs/{subject}/{session}" ]; then\n'
        f'    echo "[QC-QSIRECON] Please run QSIRecon command before QC."\n'
        f'    exit 1\n'
        f'fi\n'

        f'\n# Check that QSIRECON finished without error\n'
        f'prefix="{DERIVATIVES_DIR}/qsirecon/stdout/qsirecon_{subject}_{session}"\n'
        f'found_success=false\n'
        f'for file in $(ls $prefix*.out 2>/dev/null); do\n'
        f'    if grep -q "QSIRecon finished successfully" $file; then\n'
        f'        found_success=true\n'
        f'    fi\n'
        f'done\n'
        f'if [ "$found_success" = false ]; then\n'
        f'    echo "[QC-QSIRECON] QSIRecon did not terminate for {subject} {session}. Please run QSIRecon command before QC."\n'
        f'    exit 1\n'
        f'fi\n'
    )
    ...
    singularity_command = (
        f'\napptainer run \\\n'
        f'    --cleanenv \\\n'
        f'    -B {DERIVATIVES_DIR}/qsirecon/outputs:/data:ro \\\n'
        f'    -B {DERIVATIVES_DIR}/qc/qsirecon:/out \\\n'
...
def run(config, subject, session, job_ids=None):
    ...
    os.makedirs(f"{DERIVATIVES_DIR}/qc/qsirecon/work", exist_ok=True)

    if not utils.is_mriqc_done(config, subject, session, runtype='qsirecon'):
        path_to_script = f"{DERIVATIVES_DIR}/qc/qsirecon/scripts/qc_qsirecon_{subject}_{session}.slurm"
        generate_slurm_script(config, subject, session, path_to_script, job_ids=job_ids)
        cmd = f"sbatch {path_to_script}"
        print(f"[QC-QSIRECON] Submitting job: {cmd}")
        job_id = utils.submit_job(cmd)
        return job_id

    else:
        print(f"[QC-QSIRECON] Skip already processed MRIQC")
        print(f"[QC-QSIRECON] Performing only python command extraction for {subject}_{session}")
        try:
