...
def run_qsiprep(args, freesurfer_job_ids=None):
    """
    ...
    for subject in subjects:
        ...
        for session in sessions:
            ...
            if freesurfer_job_ids:
                header += (
                    f'#SBATCH --dependency=afterok:{":".join(freesurfer_job_ids[subject][session])}\n'
                )
            ...
            print(f"Submitting QSIprep job: {cmd}")
...
def run_qsirecon(args, qsiprep_job_ids=None):
    """
    ...
    for subject in subjects:
        ...
        for session in sessions:
            ...
            if qsiprep_job_ids:
                header += (
                    f'#SBATCH --dependency=afterok:{":".join(qsiprep_job_ids[subject][session])}\n'
                )
            ...
            # launch slurm script
            cmd = f"sbatch {path_to_script}"
            print(f"Submitting QSIrecon job: {cmd}")
            job_id = utils.submit_job(cmd)
            job_ids[subject][session] = job_id
            # subprocess.run(cmd, shell=True, check=True)
...
def main():
    ...
    general_config_file = f"{Path(__file__).parent.parent}/config.json"
    config = utils.load_config(general_config_file)
    sub_keys = ['common', 'slurm', 'qsiprep']
    for sub_key in sub_keys:
        workflow_config = config.get(sub_key, {})
        for key, value in workflow_config.items():
